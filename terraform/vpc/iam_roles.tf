...
resource "aws_iam_instance_profile" "k8master_profile" {
  name = "${aws_iam_role.k8master_role.name}"
  role = "${aws_iam_role.k8master_role.name}"
}

resource "aws_iam_role" "k8master_role" {
  name               = "k8master_role_demo"
  assume_role_policy = "${data.aws_iam_policy_document.instance-assume-role-policy.json}"
}


resource "aws_iam_role_policy_attachment" "k8master_policy_codecommit_readonly" {
  role       = "${aws_iam_role.k8master_role.name}"
  policy_arn = "arn:aws:iam::aws:policy/AWSCodeCommitReadOnly"
}

resource "aws_iam_role_policy_attachment" "k8master_policy_ec2_readonly" {
  role       = "${aws_iam_role.k8master_role.name}"
  policy_arn = "arn:aws:iam::aws:policy/AmazonEC2ReadOnlyAccess"
}
...
resource "aws_iam_instance_profile" "k8node_profile" {
  name = "${aws_iam_role.k8node_role.name}"
  role = "${aws_iam_role.k8node_role.name}"
}

resource "aws_iam_role" "k8node_role" {
  name               = "k8node_role_demo"
  assume_role_policy = "${data.aws_iam_policy_document.instance-assume-role-policy.json}"
}
